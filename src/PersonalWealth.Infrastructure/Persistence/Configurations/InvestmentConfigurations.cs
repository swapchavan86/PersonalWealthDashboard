...
public sealed class SecurityConfiguration : IEntityTypeConfiguration<Security>
{
    public void Configure(EntityTypeBuilder<Security> builder)
    {
        ...
        builder.Property(x => x.SecurityType).HasConversion<string>().HasMaxLength(30);
        builder.Property(x => x.Currency).HasMaxLength(3).IsRequired();
        builder.Property(x => x.Isin).HasMaxLength(20);
        builder.HasIndex(x => new { x.TenantId, x.Symbol }).IsUnique();
        builder.HasIndex(x => new { x.TenantId, x.Isin });
    }
}
...
public sealed class InvestmentHoldingConfiguration : IEntityTypeConfiguration<InvestmentHolding>
{
    ...
    {
        builder.ToTable("InvestmentHoldings");
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Quantity).HasPrecision(28, 10);
        builder.Property(x => x.CostBasis).HasPrecision(19, 4);
        builder.HasIndex(x => new { x.TenantId, x.InvestmentAccountId, x.SecurityId }).IsUnique();
        builder.HasOne<InvestmentAccount>().WithMany().HasForeignKey(x => x.InvestmentAccountId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne<Security>().WithMany().HasForeignKey(x => x.SecurityId).OnDelete(DeleteBehavior.Restrict);
    }
}
...
public sealed class InvestmentTransactionConfiguration : IEntityTypeConfiguration<InvestmentTransaction>
{
    public void Configure(EntityTypeBuilder<InvestmentTransaction> builder)
    {
        ...
        builder.Property(x => x.Fees).HasPrecision(19, 4);
        builder.Property(x => x.Currency).HasMaxLength(3).IsRequired();
        builder.Property(x => x.Reference).HasMaxLength(200);
        builder.HasIndex(x => new { x.TenantId, x.InvestmentAccountId, x.TransactionDate });
        builder.HasIndex(x => new { x.TenantId, x.SecurityId, x.TransactionDate });
        builder.HasOne<InvestmentAccount>().WithMany().HasForeignKey(x => x.InvestmentAccountId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne<Security>().WithMany().HasForeignKey(x => x.SecurityId).OnDelete(DeleteBehavior.Restrict);
    }
}
